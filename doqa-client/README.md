...
- `auto` (дефолт) - `api`, когда API-настройки полны, иначе `files`. Откат в `files` из-за
  отсутствующих настроек сопровождается WARNING в логе (перечисляет, чего именно не хватает):
  молчаливый откат читается как «адаптер не работает». Осознанный файловый режим - `reporting=files`;
- `off` - репортинг выключен.

Отказ DoQA не стоит результатов. Если ран не удалось установить (сервер не отвечает, токен
отвергнут, ран не создан - 401/403/422/сеть), сессия уходит в `files` на весь прогон, с WARNING,
называющим причину; если отвергнут пакет результатов уже установленного рана, в файлы пишется
только этот пакет, остальные продолжают идти по API. В `resultsDir` при этом лежит
`doqa-reporting.properties` (`sink=api|files`, `runId`, `delivered`, `fallbackResults`) - по нему
шаг загрузки пайплайна отличает «грузить нечего» от «адаптер не отчитался».
