...
- **Allure**: разметка подхватывается автоматически и без зависимости от Allure - `@AllureId`
  ...
  `@Link` и URL-значные `@Issue`/`@TmsLink` (→ типизированные ссылки), `@Description`. `@AllureId`
  дополнительно уходит в DoQA как связь 1:1 с ручным тест-кейсом с этим id - и в файловом режиме
  (метка `AS_ID`), и при отправке напрямую в API, так что привязка к кейсам переезжает без правок.
  Файловый режим эмитит Allure-совместимые результаты - существующий пайплайн загрузки продолжит
  работать.
